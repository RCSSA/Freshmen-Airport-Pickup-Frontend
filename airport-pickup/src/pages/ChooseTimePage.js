...
export default function ChooseTimePage() {
  ...
  const dataManagement = useCallback(() => {
    let idCount = 0;
    let idList = [], titles = [], dates = [], descriptions = [], pickUpStructures = [];
    ...
    for (var i = 0; i < ids.length; i++){
      ...
      if (keys2.includes("IAH")){
        idCount += 1;
        titles.push("IAH: " + sampleData[key1]["IAH"]["TotalToBePicked"] + " 人")
        dates.push(date);
  
        const keys3 = Object.keys(sampleData[key1]["IAH"]).filter((key3)=>{return (key3 !== "TotalToBePicked")});
        let description = [];
        let pickUpStructure = [];
      ...
      if (keys2.includes("HOU")){
        idCount += 1;
        titles.push("HOU: " + sampleData[key1]["HOU"]["TotalToBePicked"] + " 人")
        dates.push(date);
        
        const keys3 = Object.keys(sampleData[key1]["HOU"]).filter((key3)=>{return (key3 !== "TotalToBePicked")});
        let description = [];
        let pickUpStructure = [];
        for (var j = 0; j < keys3.length; j++){
